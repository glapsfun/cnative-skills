...
4. Helm: see `references/sibling-skills.md` for `helm-release-debug.sh`
   when the `helm` skill is installed; otherwise `helm history <release>
   -n <ns>` for releases matching the workload.
...
6. Argo CD: see `references/sibling-skills.md` for `argocd-diagnostics.sh`
   when the `argocd` skill is installed; otherwise `argocd app history
   <app>` or
