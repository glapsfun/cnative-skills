...
| Helm-managed rollback or config-fix evidence (release history, values diff) | `helm` | `scripts/helm-release-debug.sh` | `helm history <release> -n <ns>` / `helm get values` |
| Chart-authoring questions while writing a Helm-targeted config fix | `helm` | `references/03-cli-and-release-lifecycle.md` | Proceed with general Helm knowledge |
...
for, but this is now the single place sre-agent records sibling-skill
deference, so it doesn't get a second home.
