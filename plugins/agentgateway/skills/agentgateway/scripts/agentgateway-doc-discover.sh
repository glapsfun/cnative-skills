...
# Lists design docs, example configs, and schema file paths from the official
# agentgateway GitHub repository so the skill can point at current upstream
# sources instead of relying on a memorized, point-in-time snapshot.
#
# Security posture:
...
repos=(
  "agentgateway/agentgateway:design examples schema"
)

if ! command -v curl >/dev/null 2>&1; then
  echo "curl is required for agentgateway documentation discovery" >&2
  exit 1
fi
...
# Strict allowlist: repo-relative doc/example/schema paths only. Anything else
...
for path in sorted_paths[:cap]:
    print(path)
if len(sorted_paths) > cap:
    print(f"note: {len(sorted_paths) - cap} path(s) omitted by the {cap}-result cap; listing is incomplete")
if skipped:
